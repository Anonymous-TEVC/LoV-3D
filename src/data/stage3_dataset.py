"""
Stage 3 Dataset: SFT training dataset.

Single-turn format (2 turns: JSON assessment + clinical report).
"""
...
class Stage3Dataset(Dataset):
    """SFT dataset (singleturn)."""

    def __init__(self, json_path, tokenizer_path,
                 augment=False, max_seq_len=2048, seed=42,
                 **kwargs):
        """
        Args:
            json_path: path to stage1_train.json
            tokenizer_path: path to Qwen-2.5-14B
            augment: data augmentation for MRI volumes
            max_seq_len: max sequence length
            seed: random seed for reproducibility within epoch
        """
    ...
    # ------------------------------------------------------------------
    # Tokenization (singleturn, same as stage1_dataset.py)
    # ------------------------------------------------------------------
    ...
    def _tokenize_sample(self, prompt_text, response_text,
        ...
        response1_tokens = self.tokenizer.encode(
        ...
        # Truncate if needed: walk segments in reverse, truncating response segments
    ...
    def __getitem__(self, idx):
        ...
        prompt_report = sample.get('prompt_report', None)
        response_report = sample.get('response_report', None)

        # Tokenize (singleturn: 2 turns)
        input_ids, attention_mask, labels, image_token_mask = (
            self._tokenize_sample(
                prompt, response,
                prompt_report, response_report)
        )
        ...
        return {
            ...
            'has_dx_change': int(sample.get('has_dx_change', False)),
            'n_region_changes': sample.get('n_region_changes', 0),
        }
