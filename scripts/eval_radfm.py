#!/usr/bin/env python3
"""Zero-shot evaluation of RadFM on LoV-3D singleturn test set.
...
Requires the `radfm` conda env (transformers 4.28.1, torch 2.0.1).
"""
...
def build_simplified_prompt(original_prompt):
    """Replace the JSON output instructions with simplified text-based instructions."""
...
def parse_diagnosis(text):
    """Extract diagnosis from free-text output."""
...
# ============================================================================
# RadFM tokenizer setup
# ============================================================================

def get_radfm_tokenizer(tokenizer_path, max_img_size=100, image_num=32):
...
# ============================================================================
# MRI preprocessing for RadFM
# ============================================================================

def load_and_resize_mri_radfm(npy_path, target_h=512, target_w=512, target_d=32):
    ...
    vol_t = F.interpolate(vol_t, size=(target_h, target_w, target_d),
                          mode='trilinear', align_corners=False)
    vol_t = vol_t.repeat(1, 3, 1, 1, 1)
    vol_t = vol_t.unsqueeze(1)
    return vol_t  # (1, 1, 3, H, W, D)
...
def main():
    ...
    parser.add_argument('--radfm_repo', type=str, required=True,
                        help='Path to RadFM repo src/ directory')
    parser.add_argument('--lang_model_path', type=str, required=True,
                        help='Path to RadFM Language_files directory')
    parser.add_argument('--checkpoint', type=str, required=True,
                        help='Path to RadFM weights (pytorch_model.bin)')
    ...
    sys.path.insert(0, args.radfm_repo)
    ...
    tokenizer, image_padding_tokens = get_radfm_tokenizer(args.lang_model_path)
    ...
    # Monkey-patch to init LLaMA from config only (no pretrained weights needed,
    # because the RadFM checkpoint contains ALL weights including LLaMA backbone).
    _orig_init = MultiLLaMAForCausalLM.__init__
    def _init_from_config(self, lang_model_path):
        nn.Module.__init__(self)
        config = LlamaConfig.from_pretrained(lang_model_path)
        self.lang_model = _LlamaForCausalLM(config)
        from Model.RadFM.my_embedding_layer import MyEmbedding
        ...
        self.voc_size = 32000
    MultiLLaMAForCausalLM.__init__ = _init_from_config

    model = MultiLLaMAForCausalLM(lang_model_path=args.lang_model_path)
    ...
    model.eval()
    print(f"  Model loaded on {device} (fp16, {torch.cuda.memory_allocated()/1e9:.1f}GB VRAM)")
    ...
    for i, sample in enumerate(test_samples):
        ...
        # Load and resize MRI
        try:
            vision_x = load_and_resize_mri_radfm(mri_path)
            vision_x = vision_x.half().to(device)
        except Exception as e:
            print(f"  [{i+1}] SKIP {sample_id}: failed to load MRI: {e}")
            continue

        # Build simplified prompt
        simplified_prompt = build_simplified_prompt(prompt_text)

        # Replace <IMAGE> with RadFM image placeholder
        if IMAGE_SENTINEL in simplified_prompt:
            prompt_after_image = simplified_prompt.split(IMAGE_SENTINEL, 1)[1]
        else:
            prompt_after_image = simplified_prompt

        input_text = "<image>" + image_padding_tokens[0] + "</image>" + prompt_after_image

        # Tokenize
        lang_x = tokenizer(
            input_text, max_length=2048, truncation=True, return_tensors="pt"
        )['input_ids'].to(device)

        # Generate (override max_new_tokens via direct call)
        with torch.no_grad():
            model.embedding_layer.flag = 'Text'
            input_embedding, _ = model.embedding_layer(lang_x, vision_x)
            output_ids = model.lang_model.generate(
        ...
        # Debug: print first 5 samples' output
        if i < 5:
    ...
    print(f"\n{'='*70}")
    print(f"  RadFM ZERO-SHOT EVAL RESULTS (simplified prompts)")
